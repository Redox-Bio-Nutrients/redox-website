...
// validates the payload and sends the email via Microsoft Graph from
// notifications@redoxgrows.com (src/lib/graphMail.ts) to a single
// gatekeeper address.
//
// Every submission goes to the one formSettings.resourceRequestRecipient
// address, regardless of the submitted "State" — the dialog still
// collects it (shown in the email body for the recipient's own use),
// but it no longer decides who the email goes to. A per-state routing
// feature briefly existed here (2026-09-15 to 2026-09-18) but was
// reverted at Curtis's explicit request: the VPs couldn't agree who
// should own which state, so one gatekeeper now receives everything
// and disseminates it manually instead.
//
