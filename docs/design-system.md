...
- **The product owns its color.** `primaryColor` (a Sanity field, hex) flows through everything on that product's page via a `--product-color` CSS variable set once at the page root — hero, callouts, bullet dots, chart highlight bars, analysis leaders, sidebar accents, even the corner-leaf sprig gradients pick it up. Changing one hex in the CMS re-themes the whole page.
